...
Node.js 22 + Firebase Admin SDK. שירות Cloud Run אחד מטפל בנתונים ובקובצי הצילום. לא יוצרים Firebase project, מסד נוסף או Cloud Run נוסף. השרת אינו קורא מסמכים ואינו פונה לשום שירות AI; החשבוניות מוקלדות באפליקציה מהנייר.

**הסטטוס והשלמת ההפעלה:** ראו [מדריך ההפעלה באפליקציה](https://github.com/asafkiri/kiri-store-accounting/blob/main/docs/SETUP.md). קוד מוכן אינו הוכחה לפריסה בענן; SMS אמיתי מחייב בדיקה לאחר פריסה עם החשבון המורשה.
...
| `DOCUMENT_RETENTION_DAYS` | כמה ימים נשמר צילום לפני מחיקה אוטומטית מלאה; ברירת מחדל 2920 (שמונה שנים, המכסות שבע שנים מתום שנת המס), מקסימום 3650, ו־`0` מבטל את המחיקה האוטומטית |
| `PORT` | Cloud Run מספק; ברירת מחדל 8080 |
| `NODE_ENV` | `production` בענן |

משתני `OPENAI_*`, `STORE_TAX_ID`, `READINGS_PER_SCAN` ו־`MAX_SCANS_*` מגרסאות קודמות אינם נקראים עוד ואפשר להסיר אותם מה־revision.
...
`build` בודק תחביר בכל מודולי השרת והבדיקות. קוד JavaScript ESM אינו דורש transpilation. הבדיקות משתמשות ב־store מבודד, HTTP מקומי ותמונות/PDF אמיתיים שנוצרו לבדיקה; הן אינן צורכות SMS. ה־Dockerfile מריץ את הבדיקות שוב בתוך שלב build, ולכן image אינו נבנה אם הן נכשלות.
...
3. שמור את ה־service account הקיים `firebase-adminsdk-fbsvc@kiri-store-accounting.iam.gserviceaccount.com`. הפניית `OPENAI_API_KEY` אינה נדרשת עוד ואפשר להסיר אותה.
...
`src/supplier-name.js` הוא המקור המשותף לנרמול. הלקוח מחזיק העתק זהה לשימוש בלי בקשות רשת בזמן הקלדה, ו־CI באפליקציה משווה אותו למקור בשרת. הנרמול משמש להשוואה בלבד: הוא מטפל ברווחים, רישיות לטינית, גרשיים וסיומות משפטיות; השם שהמשתמש אישר נשמר כמות שהוא. אם המשתמש דוחה התאמה דומה, עליו לתת לספק האחר שם מבדיל לפני יצירה, כדי שלא להפר את כלל מניעת הכפילויות.

הכיסוי כולל יצירה מתוך חשבונית, replay אחרי אובדן תשובה, תחרות בין שתי בקשות, אימות שדות, שמירה אטומית והפעלה מחדש. בדיקת Firebase באמולטורים מריצה גם שתי בקשות HTTP מקבילות עם שמות שקולים ובודקת סנכרון של שתי הרשומות. [התנהגות טרנזקציות Firestore](https://firebase.google.com/docs/firestore/manage-data/transactions).
...
|---|---|---|
| GET | `/health` | liveness בלבד, לא בדיקת הרשאות |
...
עד 8 עמודים ו־12 MiB בסך הכול, JSON upload עד 17 MiB. MIME/signature ו־decode מאומתים: JPEG/PNG/WebP עם עמוד יחיד, PDF לא מוצפן; SVG/אנימציה/קובץ פגום נדחים. תמונה עד 80 מיליון פיקסלים כגבול פענוח. תמונות מעובדות בזו אחר זו: תיקון כיוון EXIF, הקטנה ללא הגדלה עד 2500px בצלע הארוכה ושמירה כ־JPEG באיכות 88. אין חיתוך, threshold או שינוי ניגודיות. תמונות שקופות מקבלות רקע לבן; PDF נשמר ללא שינוי. הלקוח מקטין תמונות לפני חישוב מגבלת 12 MiB, והשרת מאמת ומגביל גם לקוחות אחרים. הקובץ המוקטן הוא שנשמר ב־Storage; מספר העמודים אינו מבטיח שכל תוכן אפשרי ייכנס במגבלת הגודל. הקבצים אינם ציבוריים ואין download tokens. SHA-256 מונע upload כפול לאותו תוכן.

שדה `reviewConfirmed` נאכף בשרת: חשבונית נשמרת רק אחרי שהאדם אישר את הפרטים שהקליד; השרת אינו יכול לדעת אם אכן בדק אותם מול הנייר. `source` ו־`scanJobId` מתקבלים עדיין מדף ישן ומתעלמים מהם; כל חשבונית חדשה נרשמת כ־`manual`.

לוגים מכילים request ID, endpoint ללא query, method, duration, status וקטגוריית שגיאה. בשגיאת תשתית נוסף `errorMessage` מתיאור קבוע לפי קוד SDK מוכר; לא נרשמים הודעת SDK גולמית או stack שעשויים לכלול נתונים. אין tokens, טלפונים, SMS, תמונות או טקסט חשבונית בלוג.

גיבוי JSON אינו כולל קבצים בינאריים; לשחזור מלא צריך גם גיבוי bucket ו־Firestore מתאים. אין Import שדורס נתונים.
...
- `DOCUMENT_RETENTION_DAYS` (ברירת מחדל 2920) קובע מתי צילום נמחק מעצמו, לפי `uploadedAt`. ברירת המחדל היא שמונה שנים ולא שבע, כי חובת השמירה נספרת מתום שנת המס: חשבונית מתחילת השנה צריכה להישמר כמעט שמונה שנים מיום הצילום. צירוף אותו צילום לחשבונית נוספת מתחיל את השעון מחדש. גם צילום של חשבונית שנמחקה (tombstone) נמחק במועדו, בלי לשחזר את מספר המסמך שהחשבונית שיחררה.
- ל־Cloud Run כאן אין scheduler. המחווה נתלית על תעבורת ה־sync המאומתת, אחרי שליחת התשובה, לכל היותר פעם ב־12 שעות בכל ה־instances יחד (`system/documentPurge` כמנעול), ומטפלת עד 25 קבצים בכל פעם. `POST /api/v1/documents/purge` מריץ מחווה מיד. `documentRetentionDays` מוחזר בכל תשובת sync כדי שהאפליקציה תציג את התקופה במקום לשכפל אותה.
- קובץ שהועלה ולא צורף לשום חשבונית נמחק אף הוא כשחלפה תקופת השמירה שלו, ולכן אין צורך בניקוי ידני של טיוטות שרת.
- המחיקה מוחלטת: אין סל מחזור, אין שחזור, ואין גרסאות אובייקט ב־bucket. אחסון הצילומים בחנות משפחתית הוא שברי שקלים בחודש גם בתקופה מלאה, ולכן התקופה היא החלטת תיעוד ולא החלטת עלות: מי שרוצה תקופה קצרה יותר מוריד את `DOCUMENT_RETENTION_DAYS` (למשל 365 לשנה), ו־`0` מבטל מחיקה אוטומטית לחלוטין. receipts ב־`mutations` שומרים snapshot של החשבונית לפני השינוי, כלומר מזהי קבצים בלבד ולא תמונות.
