...
| `READINGS_PER_SCAN` | ברירת מחדל 2, מקסימום 3. כמה קריאות של אותו צילום נשלחות במקביל והתוצאות מושוות. `1` מחזיר קריאה בודדת |
...
כל צילום נקרא `READINGS_PER_SCAN` פעמים במקביל, ובקשה זהה נשלחת בכל קריאה. השדות מושווים אחד־אחד: ערך ששתי הקריאות הגיעו אליו נשמר, ערך שהן חלוקות עליו מתרוקן ומסומן ב־`uncertainFields`, כך שמסך הבדיקה שואל עליו בזרימת השאלות הרגילה. שם ספק מושווה אחרי נרמול, כך שסיומת ״בע״מ״ אינה מחלוקת; הפחתות שנויות במחלוקת נשמרות ומסומנות לאישור שורה־שורה; מזהה נשמר רק אם שתי הקריאות ראו אותו; הערות מאוחדות בלי כפילות. קריאה אחת שנכשלה אינה מכשילה את הסריקה — משתמשים בקריאות שחזרו, ומספרן מוחזר ב־`readings`. השיטה תופסת קריאה שגויה שהמודל היה בטוח בה, אך לא שגיאה שהוא חוזר עליה פעמיים, ולכן היא מעלה את הרצפה ואינה מבטיחה קריאה נכונה. **העלות במודל מוכפלת בהתאם למספר הקריאות**, בעוד מוני הסריקות סופרים תעודה אחת לכל סריקה.
...
Structured Outputs קבוע + validation עצמאי בשרת. שדות חסרים/לא ודאיים נשארים null ומסומנים `needsReview`. מע״מ אפס דורש עדות מפורשת; אין הנחת שיעור מע״מ. אי־התאמה אריתמטית מסומנת בלי לשנות אף מספר מודפס. ב־`warnings` של המודל נשארות רק הערות שהבודק יכול לענות עליהן — תיקון אחד מהשדות שנחלצו או צילום מחדש. הערה שכל נושאה הוא מספרים שאינם נשמרים (מספרי לקוח והזמנה, כתובת, מספר הקצאה, שורות פריטים, או שיוך המזהים שהשרת מכריע בעצמו מול מספר החנות) נמחקת לפני החזרת התוצאה, יחד עם כפילויות ומחרוזות ריקות. הערה שמזכירה שדה, סכום, סוג מסמך או את הצילום — וכן הערה בנושא לא מוכר — נשמרת, וההערות שהשרת עצמו מפיק מהמספרים אינן מסוננות כלל. הדפדפן דורש סקירה ולחיצת שמירה. שדה `reviewConfirmed` נאכף גם בשרת, אך כמובן שהשרת אינו יכול לדעת אם אדם אכן בדק בעיניו.
