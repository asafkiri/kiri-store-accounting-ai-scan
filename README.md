...
| `invoiceKeys` | מפתחות למניעת קליטה כפולה של אותה חשבונית: מפתח ספק+סוג+מספר (רק כשהוקלד מספר), ומפתח ספק+סוג+תאריך+סכום+מע״מ לכל חשבונית. חשבונית שסומנה `duplicateAllowed` אינה תופסת את מפתח הפרטים |
...
אותה חשבונית שנקלטת פעמיים נחסמת בשני מפתחות ב־`invoiceKeys`, בתוך אותה טרנזקציה של השמירה:

- **מספר**: ספק+סוג+מספר מנורמל, רק כשהוקלד מספר. חשבונית קיימת עם אותו מספר מחזירה `409 DUPLICATE_INVOICE` עם `details.invoiceId`.
- **פרטים**: ספק+סוג+תאריך+`totalAgorot`+`vatAgorot`, לכל חשבונית. ספק אחד אינו מוציא באותו יום שני מסמכים באותו סכום ובאותו מע״מ, ולכן זו אותה חשבונית: `409 DUPLICATE_INVOICE_DETAILS` עם `details.invoiceId`.

`duplicateAllowed: true` בשמירה מוותר על מפתח הפרטים לאותה חשבונית בלבד (לא על מפתח המספר), לשימוש כשהמשתמש קובע במפורש שמדובר בשתי חשבוניות שונות. הערך נשמר ברשומה, כך שתשלום או עריכה מאוחרים אינם נחסמים שוב; השמטת השדה שומרת את מה שנקבע, ו־`false` מחזיר את ההגנה. רק שמירה, מחיקה ושחזור נוגעים במפתחות: מחיקה משחררת את שניהם, שחזור תופס אותם מחדש ועשוי להידחות אם חשבונית אחרת תפסה אותם בינתיים, ושינוי סכום, תאריך או ספק משחרר את המפתח הישן ותופס את החדש. תשלום, ביטול תשלום ושחרור צילום אינם בודקים ואינם כותבים מפתח, ולכן חשבונית שנשמרה לפני שהמפתח הוצג אינה נחסמת בתשלום.

חשבוניות שנשמרו לפני שמפתח הפרטים הוצג אינן מחזיקות אותו עד לעדכון הבא שלהן; האפליקציה בודקת את אותו כלל מקומית מול כל החשבוניות שסונכרנו למכשיר, ולכן גם חשבונית ישנה מזוהה לפני השמירה.
