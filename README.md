...
| `DOCUMENT_RETENTION_DAYS` | כמה ימים נשמר צילום לפני מחיקה אוטומטית מלאה; ברירת מחדל 365, מקסימום 3650, ו־`0` מבטל את המחיקה האוטומטית |
...
- `DOCUMENT_RETENTION_DAYS` (ברירת מחדל 365) קובע מתי צילום נמחק מעצמו, לפי `uploadedAt`. צירוף אותו צילום לחשבונית נוספת מתחיל את השעון מחדש. גם צילום של חשבונית שנמחקה (tombstone) נמחק במועדו, בלי לשחזר את מספר המסמך שהחשבונית שיחררה.
...
- המחיקה מוחלטת: אין סל מחזור, אין שחזור, ואין גרסאות אובייקט ב־bucket. חובות שמירת מסמכים (למשל 7 שנים לפי תקנות ניהול פנקסי חשבונות) הן שיקול הגדרה: מי שצריך תקופה ארוכה יותר צריך להעלות את `DOCUMENT_RETENTION_DAYS` או לאפס אותו, ולשמור את הניירות עצמם. receipts ב־`mutations` שומרים snapshot של החשבונית לפני השינוי, כלומר מזהי קבצים בלבד ולא תמונות.
