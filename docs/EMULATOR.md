...
הבדיקה מכסה: משתמש מורשה/לא מורשה, server timestamps, יצירת חשבונית, idempotency, התנגשות גרסאות, תשלום בצ׳ק עם שני התאריכים, רישום קופה ורב־קו, שמירה/קריאה של קובץ Storage, snapshot וחסימת Firestore/Storage ישירות ל־client.

מחיקת צילום נבדקת מול Firestore ו־Storage אמיתיים: שאילתת `array-contains` בתוך טרנזקציה מונעת מחיקה של קובץ שחשבונית אחרת מצורפת אליו, מחווה של תקופת השמירה משחררת את כל החשבוניות ומוחקת את האובייקט עצמו (קיומו נבדק דרך ה־Admin SDK, כי Rules חוסמים כל קריאה ישירה), והעלאה חוזרת של אותם בתים אחרי המחיקה מקבלת אובייקט חדש ונקראת בהצלחה. כללי האמולטורים ב־`test/*.rules` נבדקים בזהות בתים מול קובצי הכללים שנפרסים מריפו האפליקציה. CI בשני הריפוזיטוריז נכשל אם הכללים שונים. חסימת Storage עם token תקף חייבת להחזיר 403; 401 אינו נחשב הוכחה לחסימת Rules.

נבדקים גם יצירת ספק וחשבונית באותה טרנזקציה, תחרות בין שמות ספקים שקולים, חסימת קליטה כפולה של אותה חשבונית לפי ספק+תאריך+סכום+מע״מ מול מסמכי `invoiceKeys` אמיתיים (כולל שמירה מפורשת עם `duplicateAllowed`), ושתי בקשות HTTP מקבילות לשמירה ולביטול של אותו mutationId. בביטול נבדק שהשמירה או הביטול מוכרעים דרך אותו receipt ב־Firestore, ושניסיון חוזר נותן את אותה הכרעה.
